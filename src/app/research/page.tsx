...
/** Non-blocking, bottom-right spinner */
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
      ...
      <ul className="mt-2 space-y-1.5 text-sm text-slate-700">
        ...
        <li>
          • You’ll get draft options with <strong>sources to assist with writing</strong>.
        </li>
        <li>• Next step: expand into a blog/cluster and repurpose to socials.</li>
      </ul>
...
export default function ResearchPage() {
  ...
  async function pollOnce(currentJobId: string) {
    try {
      const res = await fetch(`/api/keywords/${currentJobId}`, {
        ...
        credentials: "include", // carry Clerk session on tunnel
        cache: "no-store",
      });
  ...
  function scheduleNextPoll(intervalMs: number) {
    ...
    pollTimer.current = setTimeout(async () => {
      ...
      if (done) return; // stop when server says READY/FAILED

      const next = Math.min(intervalMs * 1.25, 8000); // gentle backoff
      scheduleNextPoll(next);
    }, intervalMs);
  }

  // kick the poller when we get a jobId
  useEffect(() => {
    if (!jobId) return;
  ...
  // hide spinner when the first results show or job finished
  const showGear =
    isSubmitting ||
    (!!jobId &&
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const newJobId: string = data?.jobId; // only trust jobId from our API
