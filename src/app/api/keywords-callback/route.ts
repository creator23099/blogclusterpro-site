  ...
  try {
    ...
    if (body?.uiPayload && body?.dbPayload) {
      ...
      await db.$transaction(async (tx) => {
        ...
        // Replace articles (@@unique([jobId, url]) protects at DB level)
        ...
        if (cleanArticles.length) {
          await tx.researchArticle.createMany({
            data: cleanArticles.map((a: any) => {
              const fallback = hostFromUrl(a.url);
              return {
                id: a.id,
                jobId: dbPayload.jobId,
                url: a.url,
                title: a.title ?? null,
                // 👇 fallback to hostname so UI never shows "unknown_source"
                sourceName: a.source_name ?? fallback ?? null,
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      if (!clusterRow) {
        clusterRow = await tx.cluster.create({
          data: { userId: dbUserId, title: cluster.title, niche: cluster.niche ?? "", status: "DRAFT" },
          select: { id: true },
        });
      } else {
        await tx.cluster.update({ where: { id: clusterRow.id }, data: { niche: cluster.niche ?? "" } });
      }
